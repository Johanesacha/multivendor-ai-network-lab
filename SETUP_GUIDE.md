...
- **Diagnose → Eval Harness**: pick 1 of 10 incident scenarios, click Run. Returns a
  ...
  See **Known bug #2** — the judge score is unreliable, not the keyword score.
- **Diagnose → Chaos Monkey (BGP)**: "Show BGP Status" / "Break BGP Session" /
  "Fix All Sessions" / "Random Chaos". See **Known bug #3** — on Windows this does
  **not** actually touch the routers.
...
### Bug 3 — Eval Harness LLM-judge intermittently returns `score: 0, error: true` — **not fixed, documented only**

Reproduced twice out of five scenario runs (`bgp-001` via the `orchestrator` agent,
and `intf-001` via the default agent). The judge call **does** reach the Anthropic API
and **does** consume real tokens (confirmed via `usage` in the response) — it's not an
auth or connectivity failure. Root cause, from reading `src/eval_harness.py`
(`llm_judge()`, ~line 73–124): the judge is called with `max_tokens=400`, and the judge
prompt asks Claude to return `{"score": ..., "reasoning": "..."}`. For longer agent
outputs (e.g. the verbose "ROUTING AGENT" structured format from the `orchestrator`
agent), the judge's own reasoning text can run long enough to hit the 400-token cap
*before* emitting the closing `}` — the response then contains no complete `{...}`
JSON block, the regex match in `llm_judge()` fails, and the function returns
`{"score": 0, "error": true}` even though the underlying diagnosis may have been good.

**Impact:** a `0` LLM-judge score does not reliably mean a bad diagnosis — check the
keyword score and the raw `agent_output` before trusting it. **Not patched** in this
pass (app-logic change, outside what this setup pass covers) — a one-line fix would be
raising `max_tokens` and/or instructing the judge to answer with JSON only, no prose.

### Bug 4 — Chaos Monkey ("Break BGP Session" etc.) does not actually work on Windows

Confirmed via direct API call:

```
POST /api/chaos/bgp {"action":"status"}
→ {"mode":"live", "output":"/bin/bash: C:UsershpDesktop...sim_bgp_failure.sh: No such file or directory", "returncode":127}
```

`app.py` shells out via `subprocess.run(["bash", <absolute Windows path>, action])`.
When the target program is Git Bash's `bash.exe`, its MSYS argv handling strips the
backslashes out of the Windows-style absolute path before the script ever runs (note:
every backslash is gone from the printed path, not just converted to forward slashes)
— so `bash` gets handed nonsense and fails with exit 127 ("command not found").
**Confirmed this call never touches the real routers**: `vtysh -c 'show bgp summary'`
on the target router showed uninterrupted `Up/Down` timers throughout a "Break BGP
Session" click. The UI's failure counter still updates (it doesn't strictly reflect
router state), so **don't trust the Chaos Monkey panel's displayed state on Windows** —
it can show a session as down when it's actually still up.

**Workaround:** run `network-lab/sim_bgp_failure.sh` directly from a Bash shell
(`bash network-lab/sim_bgp_failure.sh status|break|fix`) instead of through the UI/API
— this works correctly (verified) since the path-mangling only happens when Python's
`subprocess` hands the app's own absolute path to `bash.exe`.
...
- [ ] Eval Harness run returns a keyword score (LLM-judge score may occasionally read
      0/error — see Bug 3, not a setup problem)
- [ ] GAIT Audit panel shows your test events with matching token counts
- [ ] Aware that Chaos Monkey's break/fix buttons are cosmetic-only on Windows (Bug 4)
